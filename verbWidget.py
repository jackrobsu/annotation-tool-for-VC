...
class VerbWidget(QMainWindow,QObject):
    ...
    def __init__(self,pWidget,tabIndex=0):
        ...
        self.lemmatization = getButton("Lemmatisation")
        self.lemmatization.clicked.connect(self.lemmatizationButtonClickEvent)
        controlcontents = []
        self.contentWidth = 150
        self.tagWidth = 40
        (self.verb,self.verbContent) = addContent(self,'Verb', controlcontents, 0 , self.textClickSignal,tagWidth=self.tagWidth,contentWidth=self.contentWidth,noCheckBox=True,noTagTextEdit=True)
        negation = QLabel(self)
        negation.setText("Negative")
        ...
        self.originVerbWidget , self.originVerbLabel , self.originVerbTable = self._AddTableWithLabel("Base Forms",self.lemmatizationWidget)
        ...
        self.treeItemChoiceButton = getButton("Select")

        for i in range(self.treeWidgetNum) :
            tree = Tree(self.rightWindow,self,i,labels=['Verb','Description'])
        ...
        self.preButton = getButton("Last",40,buttonHeight,event=self.preButtonClickEvent)
        self.nextButton = getButton("Next",40,buttonHeight,event=self.nextButtonClickEvent)
